...
wt.start = min(wsnum);
wt.stop = max(wenum);
[Xtickmarks,XTickLabel]=findSecondMarks(wt);
...
function [Xtickmarks,Xticklabels]=findSecondMarks(timewindow);

% calculate where minute marks should be, and labels
snum = ceil(timewindow.start*86400)/86400;
enum = floor(timewindow.stop*86400)/86400;


% Number of minute marks should be no greater than 20
numSecs = (enum - snum) * 86400;
stepSecOptions = [1 2 3 5 10 15 20 30 60 120 180 240 360 480 720 1440];
c = 1;
while (numSecs / stepSecOptions(c) > 4)
	c = c + 1;
end
stepSecs = stepSecOptions(c);
Xtickmarks = snum:stepSecs/86400:enum;
Xticklabels = datestr(Xtickmarks,13);
